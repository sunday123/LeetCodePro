...
SELECT DISTINCT(u.name),c.name ,MAX(l.DATE) FROM login l,CLIENT c,USER u WHERE l.user_id =u.id AND l.client_id=c.id GROUP BY l.user_id ORDER BY u.name
```

- 查询分组里前2名

```mysql
//分区函数partition by ，不重复排序row_number()，相同同名跳跃rank(),连续相同归一组dense_rank(),NTILE(group_num)将所有记录分成group_num个组每组序号一样
